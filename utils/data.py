...
NULLKEY = "-null-"
'''
self.train_texts stores a list of sublists, where each sublist consists of text information of a single sentence, including chars, matched words.
self.train_Ids stores the ids of corresponding elements in the train_texts
'''
class Data:
    def __init__(self): 
    ...
    def build_alphabet(self, input_file):
        ...
        for idx in range(len(in_lines)):
            line = in_lines[idx]
            if len(line) > 2:
                pairs = line.strip().split() # item tag

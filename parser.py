...
def normal_to_raw(f1, f2):
    f1 = io.open(f1, 'r', encoding='utf8')
    file1= f1.readlines()
    f2 = io.open(f2, 'w', encoding='utf8')
    for line in file1:
        line = line.strip()
        for word in line:
            if len(word) == 0:
...
if __name__ == '__main__':
    ...
    #     BIO2BMOES("temp",sys.argv[3])
    # else:
    #     print("Argument error: sys.argv[1] should belongs to \"IOB2BIO/BIO2BIOES/BIOES2BIO/IOB2BIOES\"")

    f1 = "data/20160401_1_test.txt"
    f2 = "data/littletest/20160401_1_test.txt"
    normal_to_raw(f1, f2)
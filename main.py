...
def data_initialization(data, gaz_file, train_file, dev_file, test_file):
    ...
    # insert the words in ctb.50d.vec into a trie
    data.build_gaz_file(gaz_file)

    # put all possible matches with the trie into Data.gaz_alphabet
    data.build_gaz_alphabet(train_file)
    data.build_gaz_alphabet(dev_file)
    data.build_gaz_alphabet(test_file)

    #Data.keepgrowing = False
    data.fix_alphabet()
    return data
...
def batchify_with_label(input_batch_list, gpu, volatile_flag=False):
    """
        input: list of words, chars and labels, various length. [[words,biwords,chars,gaz, labels],[words,biwords,chars,labels],...]
            ...
            gaz:[[[232, 23],[2, 3]],[],[]]- there are two words in the trie that starts from words[0], index of which are 232 and 23
                    , with word length 2 and 3 respectively, there is no word that start with words[1] or words[2] in the trie.
        output:
    ...
    '''max_seq_len: num of characters in a sentence'''
    ...
    for idx, (seq, biseq, label, seqlen) in enumerate(zip(words, biwords, labels, word_seq_lengths)):
        ...
        '''sort the column, word_seg_lengths is the sorted tensor, idx stores the original index of each corresponding ele in the new tensor'''
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--savemodel', default="data/littletest/littletest")
    # parser.add_argument('--savedset', help='Dir of saved data setting', default="data/littletest/littletest.dset")
    parser.add_argument('--savemodel', default="data/resume/resume")
    ...
    '''For decode'''
    parser.add_argument('--savedset', help='Dir of saved data setting', default="data/littletest/saved_model.dset")
    parser.add_argument('--raw', default = "data/littletest/20160401_1_test.txt")
    parser.add_argument('--loadmodel',default="data/littletest/saved_model.29.model")
    parser.add_argument('--output', default = "data/littletest/out")
    ...
    if status == 'train':
        ...
        '''Find the embedding for each character
         norm: whether to turn a pre-trained char vector into a L2-norm form one. calculate oov based on word_alphabet(in fact char) and giga char embeddings
         for oov: np.random.uniform(-scale, scale, [1, embedd_dim], scale = np.sqrt(3.0 / embedd_dim), but why this scale?'''
        data.build_word_pretrain_emb(char_emb)
        #data.build_biword_pretrain_emb(bichar_emb) # default None
